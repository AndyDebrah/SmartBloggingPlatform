...
if (-not $bin) {
    ...
    if (-not $bin) {
        Write-Error "No mysql path provided - aborting."
        exit 1
    }
}
...
if (Test-Path $mysqldump) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "mysqldump returned non-zero code"
    } else {
        Write-Host "Backup written to backup.sql"
    }
} else {
...
if ($LASTEXITCODE -ne 0) {
    Write-Warning "Import returned non-zero exit code"
} else {
    Write-Host "Seed script loaded."
}
